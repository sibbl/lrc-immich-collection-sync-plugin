...
| ⚠️ `catalog:addPhoto` / `catalog:addPhotos` is a **yielding operation** — calling it inside `catalog:withWriteAccessDo` raises `AgEventLoop.yieldToScheduler called when yielding is not allowed`. Always import photos **before** the write-access block, then add the returned LrPhoto objects to a collection inside `withWriteAccessDo`. | Lightroom Classic 15.2.1 runtime error. The old `SyncServiceProvider.lua` called `addPhotos` inside `withWriteAccessDo` — this was a latent bug that newer Lightroom enforces. |
